 ...
 */
public class VFXWebNode extends Region {
    ...
    public static VFXWebNode newNode() {
        int key = NativeBinding.INSTANCE.nodes.keySet().size();
        while (NativeBinding.INSTANCE.nodes.keySet().contains(key)) {
            key++;
        }
        return new VFXWebNode(key, NativeBinding.INSTANCE);
    }

    VFXWebNode(int key, NativeBinding binding) {
        this.key = key;
